...
| B21 IMU calibration OK | Two short beeps/flashes, then A auto run starts. |
...
5. Calibration OK gives two short beeps/flashes, then the firmware starts the
   A auto mode from the original `abcd_startAutoRun()` path.
...
auto run starts, the pending auto start is cancelled.
...
```text
Build: 2026-07-05-abcd-fresh-b21-auto-start-test
```
